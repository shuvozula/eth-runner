...
if [ -f /home/root/install-nvidia-driver ]; then
	...
	sudo chmod +x *.run
	sudo ./cuda_10.0.130_410.48_linux.run -y
	echo "/usr/local/cuda-10.0/lib64" | sudo tee -a /etc/ld.so.conf.d/cuda.conf
	echo "export PATH=$PATH:/usr/local/cuda-10.0/bin" >> ~/.bash_profile
	sudo ldconfig
